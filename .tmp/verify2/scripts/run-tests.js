...
await run("holiday inn express pendleton gl reports collapse wrapped rows and skip title metadata", async () => {
    ...
    assert.deepEqual(finalAudit.rows.find((row) => row.metric_name === "ACCOMMODATION"), {
    ...
    assert.deepEqual(finalAudit.rows.find((row) => row.metric_name === "CANCELLATION CHARGE"), {
    ...
    assert.deepEqual(hotelStatistics.rows.find((row) => row.metric_name === "Rooms Available To Sell"), {
    ...
    assert.deepEqual(hotelStatistics.rows.find((row) => row.metric_name === "IHG ONE REWARDS CLUB REIMBURSEMENT"), {
    ...
    assert.deepEqual(directBillAging.rows.find((row) => row.company_name === "IHG REWARD NIGHT REIMBURSEMENTS"), {
    ...
    assert.deepEqual(directBillAging.rows.find((row) => row.company_name === "OREGON JUDICIAL DEPARTMENT"), {
    ...
    assert.deepEqual(taxReport.rows.find((row) => row.tax_name === "Tourism Promotional Assessment"), {
    ...
    assert.deepEqual(taxReport.rows.find((row) => row.transaction_number === "45040314"), {
    ...
    assert.deepEqual(taxReport.rows.find((row) => row.transaction_number === "45288564"), {
    ...
    assert.deepEqual(trialBalance.rows.find((row) => row.account_name === "CLC CheckINN T-Chek Cards"), {
