...
function main() {
    ...
    for (const snapshot of snapshots) {
        ...
        const sections = Array.from(new Set(snapshot.rows.map((row) => normalizeWhitespace(row.section)).filter(Boolean))).slice(0, 8);
...
function countCurrentMappings(snapshot) {
...
function countCollapsedMappings(rows, keyBuilder) {
    const byKey = new Set();
...
function countSpecificMetricMappings(rows, allowedFields) {
...
function buildPostingIdentityParts(row, snapshot) {
...
function buildAllTransactionCategoryLabel(row, snapshot) {
...
function isMetadataLikeRow(row) {
