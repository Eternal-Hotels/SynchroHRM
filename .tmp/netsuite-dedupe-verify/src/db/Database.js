...
export class AppDatabase {
    ...
    `);
        for (const [reportType, columns] of Object.entries(REPORT_COLUMN_MAP)) {
            const reportColumns = columns
                .filter((column) => !COMMON_EXPORT_COLUMNS.includes(column) && column !== "ingest_run_id")
    ...
    updateRunProgress(runId, summary) {
        this.db.prepare(`
      UPDATE ingest_runs
      SET messages_seen = ?,
          attachments_seen = ?,
          ...
          attachments_failed = ?
      WHERE id = ?
    `).run(summary.messagesSeen, summary.attachmentsSeen, summary.attachmentsApproved, summary.attachmentsNotApproved, summary.attachmentsArchived, summary.attachmentsParsed, summary.attachmentsDeferred, summary.attachmentsFailed, runId);
    }
    finishRun(runId, status, summary) {
      ...
      SET status = ?,
          ...
          notes = ?
      WHERE id = ?
    `).run(status, now, summary.messagesSeen, summary.attachmentsSeen, summary.attachmentsApproved, summary.attachmentsNotApproved, summary.attachmentsArchived, summary.attachmentsParsed, summary.attachmentsDeferred, summary.attachmentsFailed, JSON.stringify(summary.notes), runId);
    }
    ...
    getRun(runId) {
        const run = this.db.prepare(`SELECT * FROM ingest_runs WHERE id = ?`).get(runId);
        if (!run) {
            return null;
        }
    ...
    `).run(message.graphMessageId, message.internetMessageId, message.subject, message.senderEmail, message.receivedAt, message.webLink, now);
    }
    ...
    getNetSuiteMonetaryMappings(propertySlug, reportType) {
        return this.db.prepare(`
      SELECT
        property_slug,
        report_type,
    ...
    `).all(propertySlug, reportType);
    }
    upsertNetSuiteMonetaryMappings(records) {
        if (records.length === 0) {
            return;
        }
        const statement = this.db.prepare(`
    ...
    `);
        ...
        try {
            for (const record of records) {
                statement.run(record.propertySlug, record.reportType, record.mappingKey, record.groupLabel, record.itemLabel, record.amountField, record.amountFieldLabel, record.defaultPostingPolarity, record.postingPolarity, record.netsuiteGlCode, record.firstSeenAt, record.lastSeenAt, record.lastAttachmentId, record.lastAttachmentName, record.updatedAt);
            }
            this.db.exec("COMMIT");
        }
        catch (error) {
    ...
    getNetSuitePostingDefaults(propertySlug, reportType) {
        const row = this.db.prepare(`
      SELECT
        property_slug,
        report_type,
      ...
      WHERE property_slug = ? AND report_type = ?
    `).get(propertySlug, reportType);
        return row ?? null;
    }
    saveNetSuitePostingDefaults(record) {
        this.db.prepare(`
      ...
      ON CONFLICT(property_slug, report_type) DO UPDATE SET
        ...
        updated_at = excluded.updated_at
    `).run(record.propertySlug, record.reportType, record.balancingGlCode, record.externalIdPrefix, record.memoTemplate, record.subsidiaryId, record.currencyId, record.locationId, record.departmentId, record.classId, record.updatedAt);
    }
    insertNetSuitePostingRun(record) {
        const now = new Date().toISOString();
        this.db.prepare(`
    ...
    `).run(record.id, record.propertySlug, record.propertyName, record.reportType, record.reportTitle, record.attachmentRecordId, record.attachmentName, record.reportDate, record.status, record.externalId, JSON.stringify(record.previewPayload), record.netsuiteResponseSummary, record.netsuiteResponsePayload ? JSON.stringify(record.netsuiteResponsePayload) : "", record.errorMessage, record.createdByUsername, record.submittedAt, now, now);
    }
    updateNetSuitePostingRun(runId, update) {
      ...
      SET status = ?,
          ...
          updated_at = ?
      WHERE id = ?
    `).run(status, externalId, previewPayload, netsuiteResponseSummary, netsuiteResponsePayload, errorMessage, submittedAt, new Date().toISOString(), runId);
    }
    getNetSuitePostingRun(runId) {
        const row = this.db.prepare(`
      SELECT *
    ...
    listNetSuitePostingRuns(propertySlug, reportType, limit = 10) {
        return this.db.prepare(`
      SELECT *
      FROM netsuite_posting_runs
      WHERE property_slug = ? AND report_type = ?
      ...
      LIMIT ?
    `).all(propertySlug, reportType, limit);
    }
    getAttachmentRecord(graphMessageId, graphAttachmentId) {
        const row = this.db.prepare(`
      SELECT * FROM attachments WHERE graph_message_id = ? AND graph_attachment_id = ?
    `).get(graphMessageId, graphAttachmentId);
        return row ?? null;
    }
    listAttachmentsForReparse() {
        return this.db.prepare(`
      SELECT
        id,
        graph_message_id,
    ...
    insertAttachment(input) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(input.graphMessageId, input.graphAttachmentId, input.ingestRunId, input.internetMessageId, input.sourceMailbox, input.receivedAt, input.attachmentName, input.propertyName, input.propertySlug, input.extension, input.contentType, input.archivedPath, input.status, now, now);
        return Number(result.lastInsertRowid);
    }
    updateAttachment(recordId, input) {
        const current = this.db.prepare(`SELECT * FROM attachments WHERE id = ?`).get(recordId);
        const now = new Date().toISOString();
        this.db.prepare(`
      UPDATE attachments
      SET archived_path = ?,
          ...
          updated_at = ?
      WHERE id = ?
    `).run(pickAttachmentUpdateValue(input, "archivedPath", current.archived_path), pickAttachmentUpdateValue(input, "status", current.status), pickAttachmentUpdateValue(input, "propertyName", current.property_name), pickAttachmentUpdateValue(input, "propertySlug", current.property_slug), pickAttachmentUpdateValue(input, "reportType", current.report_type), pickAttachmentUpdateValue(input, "reportTitle", current.report_title), pickAttachmentUpdateValue(input, "reportDate", current.report_date), pickAttachmentUpdateValue(input, "parseError", current.parse_error), pickAttachmentUpdateValue(input, "parsedJsonPath", current.parsed_json_path), pickAttachmentUpdateValue(input, "quarantinePath", current.quarantine_path), pickAttachmentUpdateValue(input, "ingestRunId", current.last_ingest_run_id), now, recordId);
    }
    insertParsedReport(runId, attachmentRecordId, provenance, report) {
    ...
    getExportRows(reportType, options) {
        const clauses = [];
        const params = [];
    ...
    getAttachmentExportRows(reportType, attachmentRecordId) {
        const statement = this.db.prepare(`
      ...
      ORDER BY id
    `);
        return statement.all(attachmentRecordId);
    }
    recordExport(runId, reportType, rowCount, csvPath, latestPath, property) {
        this.db.prepare(`
    ...
    updateExportRowCount(exportId, rowCount) {
    ...
    getLatestExport(reportType, propertySlug) {
        ...
        ORDER BY id DESC
        LIMIT 1
      `).get(reportType, UNASSIGNED_PROPERTY_SLUG, propertySlug)
            : this.db.prepare(`
        SELECT * FROM export_history
    ...
    getPropertySummary(propertySlug) {
        const row = this.db.prepare(`
      SELECT
      ...
      HAVING SUM(CASE WHEN status IN ('parsed', 'deferred') THEN 1 ELSE 0 END) > 0
    `).get(UNASSIGNED_PROPERTY_SLUG, UNASSIGNED_PROPERTY_NAME, UNASSIGNED_PROPERTY_SLUG, propertySlug, UNASSIGNED_PROPERTY_SLUG);
        return row ?? null;
    }
    getPropertyAttachments(propertySlug) {
        return this.db.prepare(`
      SELECT
        id,
        graph_message_id,
    ...
    `).all(UNASSIGNED_PROPERTY_SLUG, propertySlug);
    }
    ...
    deleteParsedReportsForAttachment(attachmentRecordId) {
        this.db.exec("BEGIN");
        try {
    ...
    getPropertyReportCounts(propertySlug) {
        return this.db.prepare(`
    ...
    renameProperty(currentSlug, nextName, nextSlug) {
        const now = new Date().toISOString();
        ...
        SET property_name = ?,
            ...
            updated_at = ?
        WHERE COALESCE(property_slug, ?) = ?
      `).run(nextName, nextSlug, oldSegment, newSegment, oldSegment, newSegment, oldSegment, newSegment, now, UNASSIGNED_PROPERTY_SLUG, currentSlug);
            this.db.prepare(`
        ...
        SET property_name = ?,
            ...
            END
        WHERE COALESCE(property_slug, ?) = ?
      `).run(nextName, nextSlug, oldSegment, newSegment, oldSegment, newSegment, UNASSIGNED_PROPERTY_SLUG, currentSlug);
            for (const reportType of Object.keys(REPORT_COLUMN_MAP)) {
                this.db.prepare(`
          UPDATE ${reportType}
          SET property_name = ?,
...
function pickAttachmentUpdateValue(input, key, fallback) {
    return Object.prototype.hasOwnProperty.call(input, key)
        ? (input[key] ?? null)
        : (fallback ?? null);
}
function isSqliteDiskIoError(error) {
    return error instanceof Error && /disk I\/O error/i.test(error.message);
}
async function archiveBrokenDatabase(databasePath) {
...
function buildFallbackDatabasePath(databasePath) {
    const stamp = new Date().toISOString().replace(/[:.]/g, "-");
...
function openInitializedDatabase(databasePath) {
