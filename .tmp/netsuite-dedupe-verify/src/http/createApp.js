...
import { AuthenticationError, AuthorizationError, UserManagementError } from "../auth/AuthService.js";
...
import { REPORT_EXPORT_COLUMN_MAP, REPORT_TITLES } from "../reports.js";
import { AttachmentRetryError, IngestionOperationError, PropertyUpdateError, ReparseOperationError } from "../services/IngestionService.js";
...
export function createApp(config, database, ingestionService, authService, netSuiteConnectionService) {
    const app = express();
    ...
    app.put("/api/settings/approved-senders", (request, response) => {
        ...
        const rawPatterns = Array.isArray(request.body?.patterns)
    ...
    app.get("/api/netsuite/properties/:propertySlug", (request, response) => {
        ...
        const requestedReportType = typeof request.query.reportType === "string"
            ...
            : null;
        try {
            response.json(netSuitePostingService.getWorkspace(request.params.propertySlug, Number.isInteger(attachmentId) && attachmentId && attachmentId > 0 ? attachmentId : null, requestedReportType));
        }
        catch (error) {
    ...
    app.put("/api/netsuite/properties/:propertySlug", (request, response) => {
        ...
        try {
            response.json(netSuitePostingService.saveSetup(request.params.propertySlug, attachmentId, mappings.map((entry) => ({
    ...
    app.post("/api/netsuite/properties/:propertySlug/preview", (request, response) => {
        ...
        const defaults = isRecord(request.body?.defaults) ? request.body.defaults : {};
        try {
            response.json(netSuitePostingService.buildPreview(request.params.propertySlug, attachmentId, getResponseUser(response).username, mappings.map((entry) => ({
                mappingKey: typeof entry?.mappingKey === "string" ? entry.mappingKey : "",
                netsuiteGlCode: typeof entry?.netsuiteGlCode === "string" ? entry.netsuiteGlCode : "",
                postingPolarity: typeof entry?.postingPolarity === "string" ? entry.postingPolarity : ""
            })), defaults));
    ...
    app.get("/api/properties/:propertySlug/exports/:reportType/latest", (request, response) => {
        ...
        const latest = ingestionService.getLatestExport(reportType, propertySlug);
    ...
    app.get("/api/attachments/:attachmentId/parsed-csv", (request, response) => {
        ...
        if (!attachment || attachment.status !== "parsed" || !reportType || !REPORT_TYPES.includes(reportType)) {
            response.status(404).json({ error: `Parsed CSV for attachment ${attachmentId} was not found.` });
            return;
        }
        const rows = database.getAttachmentExportRows(reportType, attachmentId);
        ...
        response.send(toCsv(rows, REPORT_EXPORT_COLUMN_MAP[reportType]));
    });
...
function buildPropertyPayload(propertySlug, database, ingestionService) {
    const summary = database.getPropertySummary(propertySlug);
    if (!summary) {
        return null;
    }
    const counts = new Map(database.getPropertyReportCounts(propertySlug).map((entry) => [entry.report_type, entry.attachment_count]));
    return {
        property: summary,
...
function buildRunPayload(run, ingestionService) {
    if (!run) {
        return null;
    }
...
function readSessionToken(request, authService) {
...
function setSessionCookie(response, authService, token, expiresAt, secure) {
...
function clearSessionCookie(response, authService, secure) {
...
function getResponseUser(response) {
    return response.locals.authUser;
}
function requireAuthenticatedUser(response) {
...
function sendNetSuitePostingError(response, error) {
    const message = error instanceof Error ? error.message : String(error);
