...
const workbookFixtures = [
    {
        ...
        path: path.resolve("storage", "raw", "holiday-inn-express-ellensburg", "2026-05-27", "AAMkADY4MjQ5NjFkLTRjZTktNGFiZS05ZDhjLWNiMDAxMWVmZjNhYgBGAAAAAADvcm92xvZDSLd2s0tg_May_27_2026-ELNWA-Holiday_Inn_ExpressEllensburg-adjustment-a_eb13713d9174..xlsx"),
    ...
    },
    {
        ...
        path: path.resolve("storage", "raw", "holiday-inn-express-ellensburg", "2026-05-27", "AAMkADY4MjQ5NjFkLTRjZTktNGFiZS05ZDhjLWNiMDAxMWVmZjNhYgBGAAAAAADvcm92xvZDSLd2s0tg_May_27_2026-ELNWA-Holiday_Inn_ExpressEllensburg-all-transact_a9dadd162f6a..xlsx"),
    ...
    },
    {
        ...
        path: path.resolve("storage", "raw", "holiday-inn-express-ellensburg", "2026-05-27", "AAMkADY4MjQ5NjFkLTRjZTktNGFiZS05ZDhjLWNiMDAxMWVmZjNhYgBGAAAAAADvcm92xvZDSLd2s0tg_May_27_2026-ELNWA-Holiday_Inn_ExpressEllensburg-tax-report_15724b397b7a..xlsx"),
    ...
    },
    {
        ...
        path: path.resolve("storage", "raw", "holiday-inn-express-ellensburg", "2026-05-27", "AAMkADY4MjQ5NjFkLTRjZTktNGFiZS05ZDhjLWNiMDAxMWVmZjNhYgBGAAAAAADvcm92xvZDSLd2s0tg_May_27_2026-PDTOR-Holiday_Inn_ExpressPendleton-advance-depos_05901bc061aa..xlsx"),
    ...
    },
    {
        ...
        path: path.resolve("storage", "raw", "holiday-inn-express-ellensburg", "2026-05-27", "AAMkADY4MjQ5NjFkLTRjZTktNGFiZS05ZDhjLWNiMDAxMWVmZjNhYgBGAAAAAADvcm92xvZDSLd2s0tg_May_27_2026-PDTOR-Holiday_Inn_ExpressPendleton-in-house-gues_e35e6df440cc..xlsx"),
...
test("holiday inn express deferred workbook families parse into supported report types", async () => {
    for (const fixture of workbookFixtures) {
        ...
        fixture.assertParsed(parsed.rows);
    }
});
test("supported workbook families ingest as parsed instead of deferred", async () => {
    ...
    try {
        ...
        assert.ok(run);
        const savedAttachments = run.attachments;
        ...
        assert.equal(savedAttachments.filter((attachment) => attachment.property_slug === "holiday-inn-express-ellensburg").length, 3);
        assert.equal(savedAttachments.filter((attachment) => attachment.property_slug === "holiday-inn-express-pendleton").length, 2);
        assert.deepEqual(new Set(savedAttachments.map((attachment) => attachment.report_type)), new Set(workbookFixtures.map((fixture) => fixture.reportType)));
    }
    finally {
        database.close();
...
test("stored workbook repair restores misbucketed holiday inn express workbook attachments", async () => {
    ...
    try {
        ...
        const savedAttachments = initialRun.attachments;
        assert.equal(savedAttachments.length, workbookFixtures.length);
        ...
        const repairedAttachments = repairedRunRecord.attachments;
        ...
        assert.equal(repairedAttachments.filter((attachment) => attachment.property_slug === "holiday-inn-express-ellensburg").length, 3);
        assert.equal(repairedAttachments.filter((attachment) => attachment.property_slug === "holiday-inn-express-pendleton").length, 2);
        assert.deepEqual(new Set(repairedAttachments.map((attachment) => attachment.report_type)), new Set(workbookFixtures.map((fixture) => fixture.reportType)));
...
function toLegacyWindowsStoragePath(dataDir, storedPath) {
    return storedPath
