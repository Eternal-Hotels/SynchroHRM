...
import { NETSUITE_JWT_ALGORITHMS } from "../netsuite/types.js";
...
const DEFAULT_JWT_ALGORITHM = "PS256";
...
export class NetSuiteConnectionService {
    ...
    client;
    metadataCatalogDir;
    constructor(database, secretMasterKey, dataDir = "./storage", fetchImpl = fetch) {
    ...
    async exportMetadataCatalog() {
        const { settings, privateKeyPem } = this.getOperationalConnection("metadata catalog can be exported");
        const startedAt = Date.now();
        const checkedAt = new Date().toISOString();
    ...
    async resolveGlAccountIds(accountNumbers) {
        const { settings, privateKeyPem } = this.getOperationalConnection("GL accounts can be resolved");
        return this.client.resolveGlAccountIds(settings, privateKeyPem, accountNumbers);
    }
    async createJournalEntry(journalRecord) {
    ...
    getOperationalConnection(actionLabel) {
        const masterKey = this.requireMasterKey();
        const settings = this.getStoredSettings();
        assertConfiguredSettings(settings);
        const encryptedPrivateKey = this.database.getNetSuiteEncryptedPrivateKey();
        if (!encryptedPrivateKey) {
            throw new NetSuiteSettingsError(`A NetSuite private key must be saved before ${actionLabel}.`);
        }
        try {
            return {
                settings,
    ...
    async writeMetadataCatalogSnapshot(rows, schemas, context) {
...
function assertConfiguredSettings(settings) {
...
function isNetSuiteJwtAlgorithm(value) {
    return typeof value === "string" && NETSUITE_JWT_ALGORITHMS.includes(value);
}
function requireTrimmedValue(label, value) {
...
function normalizeStoredLastTest(value) {
    if (!value) {
        return null;
    }
...
function normalizeStoredLastCatalogExport(value) {
    if (!value) {
        return null;
    }
    if (value.status !== "success" && value.status !== "error") {
