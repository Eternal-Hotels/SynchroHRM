...
test("metadata catalog export writes a sorted latest CSV and per-record schema files", async () => {
    ...
    const requests = [];
    const service = new NetSuiteConnectionService(database, randomBytes(32).toString("base64"), dataDir, async (input, init) => {
    ...
    try {
        ...
        const accountSchema = JSON.parse(await readFile(path.join(String(summary.schemaDirectory), "account.schema.json"), "utf8"));
        assert.ok(accountSchema.properties);
        assert.ok("id" in accountSchema.properties);
        assert.equal((requests[2]?.init?.headers).accept, "application/schema+json");
        assert.equal((requests[2]?.init?.headers).authorization, "Bearer token-123");
    }
    finally {
        database.close();
