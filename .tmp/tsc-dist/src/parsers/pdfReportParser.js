...
function buildParsedReport(document, propertyName, propertySlug, reportDate) {
...
function detectReportType(document) {
...
function isChoiceAuditPacket(document) {
    const titles = new Set(document.lines
        .map((line) => line.text.trim())
        .filter((text) => (CHOICE_AUDIT_PACKET_PAGE_TITLES.includes(text))));
...
function isAllNightAuditPacket(document) {
    const titles = new Set(document.lines
        .map((line) => line.text.trim())
        .filter((text) => (ALL_NIGHT_AUDIT_PACKET_TITLES.includes(text))));
    return titles.has("Reservation Activity Report") && titles.size >= 2;
}
function extractReportDate(document) {
...
function parseMonthDayWithReferenceYear(value, referenceDate) {
...
function extractPropertyName(document) {
    ...
    for (let index = 0; index < candidates.length; index += 1) {
        ...
        const stripped = normalizeDetectedPropertyName(line
...
function extractVisibleReportTitle(document) {
    const candidates = document.lines
        .filter((line) => line.pageNumber === 1)
        .slice(0, 12)
        .map((line) => line.text.trim())
...
function extractNamedPropertyHeader(document) {
...
function extractAllNightAuditBusinessDate(document) {
...
function extractChoiceAuditPacketBusinessDate(document) {
...
export function parseChoiceAuditPacketDocument(document) {
    return parseChoiceAuditPacket(document);
}
function parseChoiceAuditPacket(document) {
    const rows = [];
...
function parseChoiceAuditPacketPage(page, reportName, document) {
    switch (reportName) {
        case "A/R Aging":
...
function parseChoiceAuditPacketGenericPage(page, reportName) {
    const rows = [];
    let section = null;
    for (const line of page) {
        ...
        rows.push(createChoiceAuditRow(line, reportName, section, isChoiceAuditPacketTotalLine(text) ? "total" : "detail", text));
    }
    return rows;
}
function parseChoiceArAgingPage(page, reportName) {
    const rows = [];
    for (const line of page) {
        const text = line.text.trim();
...
function parseChoiceAdvanceDepositLedgerPage(page, reportName, document) {
    const rows = [];
...
function parseChoiceCancellationListPage(page, reportName, document) {
    const rows = [];
    const referenceDate = extractChoiceAuditPacketBusinessDate(document);
    ...
    const blocks = collectLineBlocks(detailLines, (line) => /^\d+$/.test(sliceText(line, 38, 101)), (line) => /^Total Cancellations:/i.test(line.text.trim()));
...
function parseChoiceCityTaxReportPage(page, reportName) {
    const rows = [];
...
function parseChoiceComplimentaryRoomsPage(page, reportName) {
    const rows = [];
    for (const line of page) {
        const text = line.text.trim();
...
function parseChoiceFinalTransactionCloseoutPage(page, reportName) {
    const rows = [];
    let section = null;
    let pendingLabel = [];
    ...
    for (const line of page) {
        ...
        const row = createChoiceAuditRow(line, reportName, section, metricName && /^Total For /i.test(metricName) ? "total" : "detail", buildChoiceMetricLineText(pendingLabel, text));
        pendingLabel = [];
        if (metricName) {
...
function parseChoiceGuestLedgerPage(page, reportName, document) {
    const rows = [];
    const referenceDate = extractChoiceAuditPacketBusinessDate(document);
    let section = null;
    for (const line of page) {
...
function parseChoiceHotelJournalDetailPage(page, reportName, document) {
    const rows = [];
    const referenceDate = extractChoiceAuditPacketBusinessDate(document);
...
function parseChoiceHotelJournalSummaryPage(page, reportName) {
    const rows = [];
    for (const line of page) {
        const text = line.text.trim();
        if (!text
        ...
        const row = createChoiceAuditRow(line, reportName, null, /^Today's Total:/i.test(text) ? "total" : "detail", text);
...
function parseChoiceHotelStatisticsPage(page, reportName) {
    const rows = [];
    let section = null;
    for (const line of page) {
        const text = line.text.trim();
...
function parseChoiceRevenueByRateCodePage(page, reportName) {
    const rows = [];
    for (const line of page) {
        const text = line.text.trim();
        if (!text
        ...
        const row = createChoiceAuditRow(line, reportName, null, metric.label && /^Total/i.test(metric.label) ? "total" : "detail", text);
        row.metric_name = metric.label ?? null;
...
function parseChoiceTaxExemptReportPage(page, reportName) {
    const rows = [];
    let section = null;
    for (const line of page) {
        const text = line.text.trim();
...
function buildChoiceHotelJournalDetailRow(block, reportName, section, referenceDate) {
    const base = block[0];
...
function createChoiceAuditRow(line, reportName, section, rowKind, lineText) {
    return {
...
function setChoiceMetricValues(row, values) {
    for (let index = 0; index < values.length; index += 1) {
        row[`value_${index + 1}`] = values[index] ?? null;
    }
}
function extractChoiceFlexibleMetricLine(text, maxValues) {
...
function splitChoiceLabelAndCode(value) {
    const normalized = normalizeText(value);
    if (!normalized) {
...
function extractChoiceCityTaxMonthLabels(page) {
...
function extractChoiceHotelStatisticsSection(text) {
...
function looksLikeChoiceJournalDetailStart(line, referenceDate) {
    return Boolean(parseMonthDayWithReferenceYear(sliceText(line, 38, 90), referenceDate));
}
function formatChoiceBlockText(lines) {
    return lines
        .map((line) => line.text.trim())
        .filter(Boolean)
        .join(" | ");
}
function buildChoiceMetricLineText(pendingLabel, text) {
    return [...pendingLabel, text].filter(Boolean).join(" | ");
}
function isChoiceAuditPacketCommonHeaderOrFooter(text, reportName) {
...
function parseAllNightAuditReport(document) {
...
function parseStandaloneAllNightAuditListing(document) {
    const rows = [];
...
function extractStandaloneAllNightAuditTitle(document) {
    ...
    for (const line of firstPageLines) {
        if (ALL_NIGHT_AUDIT_STANDALONE_TITLES.includes(line)) {
            return line;
        }
    }
    return null;
}
function isAllNightStandaloneHeadingOrMeta(text) {
    return isAllNightAuditHeaderOrFooter(text)
        || text === "???"
        || ALL_NIGHT_AUDIT_STANDALONE_TITLES.includes(text)
        || /^Date:\s+/i.test(text)
...
function parseAllNightAuditBusinessOnBooks(pages) {
    const rows = [];
    for (const page of pages) {
...
function parseAllNightAuditCancellationList(lines) {
    const rows = [];
    let block = [];
    const flushBlock = () => {
        if (block.length === 0) {
...
function parseAllNightAuditHotelStatistics(lines) {
    const rows = [];
    let section = null;
    for (const line of lines) {
        const text = line.text;
...
function parseAllNightAuditNoShow(lines) {
...
function parseAllNightAuditRateDiscrepancy(pages) {
    const rows = [];
    let block = [];
    const flushBlock = () => {
        if (block.length === 0) {
...
function parseAllNightAuditReservationActivity(pages) {
    const rows = [];
    let block = [];
    const flushBlock = () => {
        if (block.length === 0) {
...
function buildAllNightAuditMetricRow(reportName, section, metricName, value1, extra) {
    return {
        report_name: reportName,
        section,
        row_kind: "metric",
...
function hasPageTitle(lines, title) {
    return lines.some((line) => line.text.trim() === title);
}
function extractChoiceAuditPacketPageTitle(lines) {
    for (const line of lines) {
        const text = line.text.trim();
        if (CHOICE_AUDIT_PACKET_PAGE_TITLES.includes(text)
...
function isChoiceAuditPacketHeaderOrFooter(text, reportName) {
...
function isChoiceAuditPacketSectionLine(text) {
...
function isChoiceAuditPacketTotalLine(text) {
...
function parseHistoryAndForecast(document) {
    const rows = [];
    let section = null;
    for (const line of document.lines) {
        const text = line.text;
...
function parseManagerFlash(document) {
    const rows = [];
    for (const line of document.lines) {
        const text = line.text;
...
function parseReservationsMadeYesterday(document) {
    const rows = [];
    let lastRow = null;
    for (const line of document.lines) {
        const text = line.text;
...
function parseZeroRateRooms(document) {
    const rows = [];
    let lastRow = null;
    for (const line of document.lines) {
        const text = line.text;
...
function parseArDetailedAging(document) {
    const rows = [];
...
function parseRateChangeReport(document) {
    const rows = [];
    for (const line of document.lines) {
        const text = line.text;
...
function parseBestWesternDailyReport(document) {
    const pages = new Map();
    for (const line of document.lines) {
...
function extractBestWesternBusinessDate(document) {
...
function parseBestWesternStatisticalRecap(lines) {
    const rows = [];
    let inSection = false;
    for (const line of lines) {
        const text = line.text;
...
function parseBestWesternDepositTotals(lines) {
    const rows = [];
    let inSection = false;
    let subsection = null;
    for (const line of lines) {
...
function parseBestWesternLedgerSummary(lines) {
    const rows = [];
    let inSection = false;
    for (const line of lines) {
        const text = line.text;
...
function parseBestWesternDetailListing(lines) {
    const rows = [];
    let subsection = null;
...
function parseBestWesternDetailSummary(lines) {
    const rows = [];
    let subsection = null;
    for (const line of lines) {
        const text = line.text;
...
function parseRoomTaxListing(document, referenceDate) {
    const rows = [];
    for (const line of document.lines) {
        const text = line.text;
        if (!text
...
function parseDailyTransactionLog(document, referenceDate) {
    const rows = [];
    let prefixLine = null;
    for (const line of document.lines) {
        const text = line.text;
...
function parseCreditCardTransactions(document) {
    const rows = [];
    let prefixLine = null;
    for (const line of document.lines) {
        const text = line.text;
...
function parseOperatorTransactions(document, referenceDate) {
    const rows = [];
    let prefixLine = null;
    for (const line of document.lines) {
        const text = line.text;
...
function parseAdjustmentRefundActivity(document) {
    const detailLines = [];
    const summaryLines = [];
    let section = null;
    for (const line of document.lines) {
    ...
    const detailBlocks = collectLineBlocks(detailLines.filter((line) => (!line.text.startsWith("Date Time Transaction")
        && !line.text.startsWith("Type Name User")
        && line.text !== "Code"
        && !/^Totals\b/.test(line.text))), (line) => isNamedDateTimeLine(line.text));
    ...
    const summaryBlocks = collectLineBlocks(summaryLines.filter((line) => !line.text.startsWith("Type Name User")), (line) => /^(Charge Type And User|Reason Code)\b/.test(line.text));
...
function parseAllTransactions(document) {
    let section = null;
    const dataLines = [];
    for (const line of document.lines) {
        const text = line.text;
        ...
        if (text.startsWith("Date Time Confirmatio")
...
function parseClosedFolioBalances(document) {
    return parseHolidayFolioBalanceReport(document, "closed");
}
function parseInHouseGuestFolioBalances(document) {
    return parseHolidayFolioBalanceReport(document, "in-house");
}
function parseHolidayFolioBalanceReport(document, mode) {
    const rows = [];
    let section = null;
    let detailBlock = [];
    let totalBlock = [];
    ...
    for (const line of document.lines) {
        ...
        if (text.startsWith("Confirmatio Group Room Guest")
...
function parseRateReport(document) {
...
function parseReservationListing(document) {
    const rows = [];
    let section = null;
    for (const line of document.lines) {
        const text = line.text;
        ...
        if (text.startsWith("Market Local Market Confirmation")
...
function parseTrialBalanceReport(document) {
    const lines = document.lines.filter((line) => {
        const text = line.text;
        return Boolean(text)
            && !shouldSkipCommonLine(text)
    ...
    const blocks = collectLineBlocks(lines, (line) => (/^(?:ASSET|LIABILITY|EQUITY|REVENUE|EXPENSE)\b/.test(line.text)
...
function parseAdvanceDepositActivity(document) {
    ...
    const blocks = collectLineBlocks(lines, (line) => /^\d{8}$/.test(sliceText(line, 40, 80)) && Boolean(parseShortDate(sliceText(line, 190, 240))), (line) => /^Totals\b/.test(line.text));
    return blocks.map((block) => ({
...
function parseBookedReservations(document) {
    const lines = filterDataLines(document, new Set([
        "Booked Reservations",
    ...
    const blocks = collectLineBlocks(lines, (line) => Boolean(parseShortDate(sliceText(line, 30, 82))) && /^\d{8}$/.test(sliceText(line, 108, 150)), (line) => /^Totals\b/.test(line.text));
    return blocks.map((block) => ({
...
function parseDirectBillAging(document) {
    const rows = [];
    let section = null;
    let currentBlock = [];
    let leadingLines = [];
...
function parseDirectBillLedger(document) {
    const lines = filterDataLines(document, new Set([
    ...
    const blocks = collectLineBlocks(lines, (line) => /^\d{8}$/.test(sliceText(line, 86, 126)) && Boolean(parseShortDate(sliceText(line, 198, 242))), (line) => /^Totals\b/.test(line.text));
    return blocks.map((block) => ({
...
function parseFinalAudit(document) {
    const rows = [];
    let section = null;
    let currentBlock = [];
    let leadingLines = [];
...
function parseHighBalanceReport(document) {
    const lines = filterDataLines(document, new Set([
        "High Balance Report",
    ...
    const blocks = collectLineBlocks(lines, (line) => /^\d+$/.test(sliceText(line, 40, 60)) && Boolean(parseShortDate(sliceText(line, 212, 258))), (line) => /^Totals\b/.test(line.text));
    return blocks.map((block) => ({
...
function parseHotelStatistics(document) {
    const rows = [];
    let section = null;
    let currentBlock = [];
    let leadingLines = [];
...
function parseMaintenanceSummary(document) {
    const rows = [];
    let pendingReason = [];
    for (const line of document.lines) {
        const text = line.text;
...
function parseOccupancyForecast(document) {
    const rows = [];
    for (const line of document.lines) {
...
function parseRateOverride(document) {
    const lines = filterDataLines(document, new Set([
        "Rate Override",
    ...
    const blocks = collectLineBlocks(lines, (line) => Boolean(parseShortDate(sliceText(line, 26, 68))) && /^\d{8}$/.test(sliceText(line, 186, 226)), (line) => /^Totals\b/.test(line.text));
    return blocks.map((block) => ({
...
function parseTaxReport(document) {
    const rows = [];
    let section = "Summary";
    let detailBuffer = [];
    const flushDetailBuffer = () => {
    ...
    for (const line of document.lines) {
        ...
        if (text.startsWith("Tax Name Total Revenue")
...
function firstNonEmptySlice(lines, minX, maxX) {
    for (const line of lines) {
...
function joinSlices(lines, minX, maxX) {
    const parts = lines
...
function normalizeFlexibleNumeric(value) {
    if (!value) {
        return null;
    }
    let cleaned = value.trim().replace(/\s+/g, "");
...
function extractBestWesternMetricLine(text, maxValues) {
...
function extractMetricLine(text, maxValues) {
    const normalized = text.replace(/(\d)\s+%/g, "$1%");
    const tokenPattern = /-?\$?\d[\d,]*(?:\.\d+)?%?/g;
...
function isDirectBillAgingHeader(text) {
    return text === "Company Name"
        || text === "Company Code"
        || text === "Current"
        || text.startsWith("Over ");
}
function getHotelStatisticsSectionHeader(text) {
...
function getFinalAuditSectionHeader(text) {
...
function getFinalAuditValueLimit(section) {
...
function isCreditCardDetailLine(line) {
...
function sliceOptionalText(line, minX, maxX) {
...
function determineBestWesternDetailRowKind(line) {
    const text = line.text;
    if (!text) {
        return null;
    }
...
function shouldSkipCommonLine(text) {
...
function isNonPropertyHeader(text) {
...
function shouldSkipVisibleTitleLine(text) {
...
function shouldMergeWrappedBoundaryToken(existing, nextToken, partTokenCount) {
    ...
    if (/^[a-z][A-Za-z0-9_()/-]*$/.test(nextToken)
