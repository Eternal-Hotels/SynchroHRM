...
test("example data ingests end to end into storage, sqlite, and csv exports", async () => {
  ...
  try {
    ...
    assert.equal(
      attachments.filter((attachment) => attachment.property_slug === "red-lion-hotel-pasco-airport-and-conference-center").length,
      8
    );
    assert.equal(
      attachments.filter((attachment) => attachment.property_slug === "best-western-pendleton-inn").length,
      4
    );
    assert.ok(
      attachments.every((attachment) => (
        attachment.archived_path.includes(path.join("raw", "red-lion-hotel-pasco-airport-and-conference-center"))
    ...
    const reparsedAttachments = reparsedRunRecord.attachments as Array<{ property_slug: string; status: string }>;
