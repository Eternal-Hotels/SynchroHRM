...
If the Fedora host does not use the same DNS servers as your clients, test the proxy locally with:

```bash
curl -k --resolve synchro.eternalhotels.com:443:127.0.0.1 https://synchro.eternalhotels.com/health
```
...
The install script opens `http` and `https` on every active `firewalld` zone. If your server uses a nonstandard zone assignment, you can override it explicitly, for example:

```bash
sudo FIREWALL_ZONES=public bash /opt/synchrohrm/deploy/fedora/install-server.sh
```
