...
const WORKBOOK_REPORT_TYPES = new Map<string, ReportType>([
...
export class WorkbookReportParser {
  ...
  async analyze(bytes: Buffer): Promise<WorkbookAnalysis> {
    const rows = extractWorkbookRows(bytes);
    const reportTitle = extractWorkbookReportTitle(rows);
    const hasSupportedTitle = Boolean(reportTitle && WORKBOOK_REPORT_TYPES.has(reportTitle.toLowerCase()));
    const propertyName = hasSupportedTitle ? extractWorkbookPropertyName(rows) : null;
    const propertySlug = hasSupportedTitle ? slugifyPropertyName(propertyName) : null;
    const reportDate = hasSupportedTitle ? extractWorkbookReportDate(rows) : null;
...
function extractWorkbookRows(bytes: Buffer): WorkbookRow[] {
...
function buildParsedWorkbookReport(
  rows: WorkbookRow[],
  propertyName: string | null,
  propertySlug: string | null,
  reportDate: string | null
...
function detectWorkbookReportType(rows: WorkbookRow[]): ReportType {
...
function extractWorkbookPropertyName(rows: WorkbookRow[]): string | null {
  return normalizeDetectedPropertyName(cell(rows[0], 0), cell(rows[1], 0));
}

function extractWorkbookReportTitle(rows: WorkbookRow[]): string | null {
...
function extractWorkbookReportDate(rows: WorkbookRow[]): string | null {
...
function parseAdjustmentWorkbook(rows: WorkbookRow[]): Array<Record<string, string | null>> {
  const parsed: Array<Record<string, string | null>> = [];
...
function parseAllTransactionsWorkbook(rows: WorkbookRow[]): Array<Record<string, string | null>> {
  const parsed: Array<Record<string, string | null>> = [];
  let section: string | null = null;
...
function parseTaxWorkbook(rows: WorkbookRow[]): Array<Record<string, string | null>> {
  const parsed: Array<Record<string, string | null>> = [];
  let section: string | null = null;
...
function parseAdvanceDepositWorkbook(rows: WorkbookRow[]): Array<Record<string, string | null>> {
  const parsed: Array<Record<string, string | null>> = [];
  let section: string | null = null;
...
function parseInHouseWorkbook(rows: WorkbookRow[]): Array<Record<string, string | null>> {
  const parsed: Array<Record<string, string | null>> = [];
  let section: string | null = null;
...
function isAdjustmentHeaderRow(row: WorkbookRow, section: string): boolean {
...
function isAllTransactionsHeaderRow(row: WorkbookRow): boolean {
  const first = cell(row, 1);
  return first === "Date";
}

function isTaxHeaderRow(row: WorkbookRow): boolean {
  const first = cell(row, 1);
  return first === "Tax Name" || first === "Transaction Number" || first === "Charge Type Code";
}

function isAdvanceHeaderRow(row: WorkbookRow): boolean {
  const first = cell(row, 1);
...
function isInHouseHeaderRow(row: WorkbookRow, section: string): boolean {
  const first = cell(row, 1);
  if (!first) {
    return false;
  }
...
function cell(row: WorkbookRow | undefined, index: number): string | null {
  return row?.[index] ?? null;
}

function firstNonEmptyCell(row: WorkbookRow): string | null {
  for (const value of row) {
    if (value) {
      return value;
    }
...
function hasAnyCurrencyCells(row: WorkbookRow): boolean {
  return row.some((value) => Boolean(normalizeCurrencyAmount(value)));
}

function parseWorkbookDate(value: string | null): string | null {
  return parseShortDate(value) ?? parseLongDate(value);
}

function firstPopulatedValue(values: Array<string | null>): string | null {
  return values.find((value) => Boolean(value)) ?? null;
}

function isNumericCell(value: string | null): boolean {
  return Boolean(value && /^\d+$/.test(value));
}

function buildNote(entries: Array<[string, string | null]>): string | null {
  const parts = entries
